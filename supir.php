...
if (isset($_GET['id'])) {
    $id = intval($_GET['id']);
    mysqli_query($koneksi, "DELETE FROM supir WHERE id_supir = $id");
    header("Location: supir.php");
    exit;
}
// Ambil statistik online, offline, total akun
$total = mysqli_query($koneksi, "SELECT COUNT(*) as total FROM supir");
$data_total = mysqli_fetch_assoc($total);

$online = mysqli_query($koneksi, "SELECT COUNT(*) as online FROM supir WHERE status='online'");
$data_online = mysqli_fetch_assoc($online);

$offline = mysqli_query($koneksi, "SELECT COUNT(*) as offline FROM supir WHERE status='offline'");
$data_offline = mysqli_fetch_assoc($offline);

// Ambil semua data user
$users = mysqli_query($koneksi, "SELECT * FROM supir ORDER BY id_supir ASC");
?>
  ...
  <div class="background">
    ...
    <div class="sidebar">
      <div class="sidebar-top">
        ...
        <button class="btn supir" onclick="window.location.href='supir.php'">Statistik supir</button>
...
<div class="dashboard-cards">
 ...
 <div class="top-bar">
  ...
  <div class="right-action">
    <button class="action" onclick="window.location.href='create_supir.php'">Create</button>
  </div>
</div>
      ...
      <table>
        ...
        <tbody>
          ...
          <tr>
            ...
            <td><?= htmlspecialchars($user['bus']) ?></td>
            <td>
              <div class="action-icons">
                <a href="edit_supir.php?id=<?= $user['id_supir'] ?>" title="Edit"><i class="fas fa-pen-to-square"></i></a>
                <a href="supir.php?id=<?= $user['id_supir'] ?>" title="Hapus" onclick="return confirm('Yakin ingin menghapus user ini?')"><i class="fas fa-trash-alt"></i></a>
              </div>
            </td>
          </tr>
          <?php endwhile; ?>
